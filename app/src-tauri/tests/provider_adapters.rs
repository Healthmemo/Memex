...
async fn anthropic_chat_parses_text_content_and_usage() {
    ...
    std::env::set_var("MEMEX_ANTHROPIC_URL", format!("{}/v1/messages", server.uri()));
...
async fn openai_list_models_filters_to_chat_capable() {
    let server = MockServer::start().await;
    std::env::set_var("MEMEX_OPENAI_MODELS_URL", format!("{}/v1/models", server.uri()));
...
async fn ollama_list_models_parses_tags() {
    ...
    assert_eq!(models, vec!["llama3.2:1b".to_string(), "qwen2.5:7b".to_string()]);
}
...
async fn anthropic_static_model_catalog() {
    let models = list_models("anthropic-api", Some("k".into())).await.unwrap();
